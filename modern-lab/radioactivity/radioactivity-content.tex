...
We have seen that a process of detection of a particle in a G-M tube requires a
...
How is the resolving time determined?  A simple method to be carried out in this
...
Note that $N$ is the number of undecayed nuclei, and will decrease as time t
...
reached.  The count rate for the parent is still close to $R_0$\sidenote{Why?}
...
\item ({\em Optional})\sidenote{May be worth extra credit; ask your instructor.} 
  ...
  $R_{b}$).\sidenote{Note: Please do not spend more than one hour on this
    question.  Carrying through the calculation while consistently applying the
    approximations involved is not entirely straightforward.}
\end{enumerate}
...
\noindent {\bf CAUTION: DO NOT HANDLE ANY OF THE RADIOACTIVE SOURCES.}
...
\item To determine the resolving time of your G-M tube, measure the count rate
  ...
  times.  Sources 1 and 2 are ${}^{204}{\rm Tl}$.\sidenote{{\em Note}: Before
    proceeding, perform the following check on your data: Assume that the
    resolving time $t_{r}$ of your G-M tube is 300 microseconds $ = 3 \times
    10^{-4}$ seconds.  Evaluate the quantity $R_{b} t_{r}$ numerically and show
    your results to your instructor. Does the relation $ R_{b} t_{r} \ll 1$
    hold?  If so, you may go on to the rest of the experiment.  If not, you must
    repeat the measurements in this step and place the radioactive sources
    farther from the G-M tube.  It is important that all count rates are small
    enough so that $R_{b}t_{r} \ll 1$, since our treatment of resolving time
...
\item Correct the \bam count rate data (from Procedure step 6), first for
  ...
  about the mathematical relationship between $R_{c}$ and $t$.\sidenote{{\em
      Note}: All count rates must be in the same units, e.g. counts/minute.  It
    may occur that for the last data points, the corrected count rates might be
    slightly negative.  Those points must be eliminated from the data in order
    to do the required data analysis.}
...
\item Perform an appropriate curve fit on the $R_{c}$ vs. $t$ data from
  ...
  %by applying the Excel function {\tt LINEST} to the decay data in the form
  %$\ln(R_{c})$ as a function of $t$.
...
%{\em Note}: If {\tt LINEST} is not available, use the $\ln(R_{c})$ versus $t$
%graph and perform ``by hand'' another curve fit.  The difference in slopes for
%the two fits (one done by Excel and the other by hand) is a rough measure of the
%uncertainty in the slope and hence, the uncertainty in $K$.
