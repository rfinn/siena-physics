...
the range 400-700 nm.\sidenote{Note: Common units for visible light wavelengths
  are the Angstrom ({\AA}), $1 {\rm \AA} = 1 \times 10^{-10} {\rm m}$ and the
  nanometer (nm), $1 {\rm nm} = 1 \times 10^{-9} {\rm m}$.}
...
spectrum of hydrogen has that behavior.\sidenote{An initial semi-classical
  understanding first came in 1913 with the Bohr model of the hydrogen atom,
  and, ultimately, with the quantum-mechanical description of the hydrogen atom
  provided by Schr\"{o}dinger's equation.}
\medskip
...
$c$ is the speed of light.\sidenote{Planck's constant $h = 6.63 \times 10^{-34}
  {\rm J} {\rm s}$.}
...
The deflection angle ${\theta}$ entering Eq.~\ref{eq:gratingmaxima} is then
determined by: 
\begin{equation}
...
\item Align the telescope and collimator. Open the collimator slit to its
  ...
  the telescope is in focus and aligned vertically.\sidenote{During this
    adjustment, the sleeve should be held in position so that its ``V point''
    fits into the V-shaped notch on the collimator mounting.} Now tighten the
...
\item Narrow the slit and align the telescope so that the slit appears
  ...
  purpose).
  %Consult the instructor for assistance if needed.
...
\item Replace the prism clamp with the grating holder and mount a grating on the
  ...
  violet).\sidenote{Important note: Check your angular scale reading for the
    zeroth order.  Is it zero?  If you have not zeroed the angular scale as
    described in the procedure above, the zeroth order angle must also be
    recorded, and then the deflection angle ${\theta}$ for non-zero orders
    obtained using Eq.~\ref{eq:calib}. This correction will also be needed in
    steps 2 and 3 below.}
...
\item Now use the mercury vapor lamp or discharge tube.\sidenote{Note: Spectral
  lines from this source tend to be dimmer, so your eyes should be well
  dark-adapted.  Also, it helps to widen the collimator slit somewhat.} Starting
