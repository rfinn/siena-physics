...
particle\sidenote{Either an electron ($e^-$) or a positron ($e^+$)---in this
  case an electron.}  and a neutrino ($\nu$ or $\bar{\nu}$).  These particles
...
K-shell electrons.\sidenote{The K-shell electrons form the innermost electronic
  shell and are closest to the nucleus.}  The probability is smaller for
...
If one plots the number of electrons emitted by \cs as a function of their
...
show that Eq.~\ref{eq:2} can be written as:\sidenote{Hint: Start with
  Eq.~\ref{eq:2} and show that if $B = 1.0~{\rm T}$ and $r = 1.0~{\rm m}$, then
  $pc = 3.0 \times 10^8~{\rm eV} = 300~{\rm MeV}$.  Hence, show that $pc~{\rm
    [MeV]} ~=~ 300 B~{\rm [T]}~r~{\rm [m]}$. And remember that $1~{\rm MeV} =
  10^6~{\rm eV}$.}
...
probe in the center port of the spectrometer.\sidenote{To carry out this
  measurement correctly, the flat side of the gaussmeter probe must be oriented
  perpendicular to the direction of the magnetic field.  You can check the
  orientation by rotating the probe slightly.  In the proper orientation, the
  gaussmeter reading will be at a maximum.}
...
\item Tabulate the values of $I$ and the corresponding measured values of $B$.
  Express $B$ in Tesla (the SI unit).\sidenote{Recall that $1 {\rm T } ~=~
    10^4~{\rm G} = 10~{\rm kG}$.}  Plot $B$ versus $I$ and perform a linear
...
\item It is convenient in these calculations to express all energies in units of
  MeV and to express the momentum $p$ in MeV/c.\sidenote{The result of
    Preliminary Question 1 will be helpful here and also recall that the rest
    energy of the electron, $mc^2 = 0.511 {\rm MeV}$.}
\end{enumerate}
...
electron emitted in the beta decay.\sidenote{Hint: See the discussion just
  preceding and following Eq.~\ref{eq:kurie}.}
